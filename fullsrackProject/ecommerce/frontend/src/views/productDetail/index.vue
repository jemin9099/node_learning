...
import categoryWiseProduct from '@/components/home/categoryWiseProduct.vue';
...
const getCategory = async () => {
    const { data, status } = await axios.get(SummaryApi.productById.url(route.params.id), { headers: authHeaders })
    if (status === 200) {        
        product.value = {
            ...data.data,
...
<template>
    <div class="container mx-auto p-3">
        <h2 class="text-2xl font-medium leading-tight ml-3 mb-3">Product Details</h2>
                ...
                <div class="flex flex-wrap -mx-4">
                    ...
                    <div class="w-full md:w-1/2 px-4 mb-8">
                        <img :src="selectedImage"
                            alt="Product" class="w-full h-auto rounded-lg shadow-sm mb-4" id="mainImage">
                        <div class="flex gap-4 py-4 justify-center overflow-x-auto" >
                            <img :src="image" alt="Thumbnail 1" v-for="image in product?.image"
                                class="size-16 sm:size-20 object-cover rounded-md cursor-pointer opacity-60 hover:opacity-100 transition duration-300" @click="getImage(image)">
                    ...
                    <div class="w-full md:w-1/2 px-4">
                        ...
                        <p class="text-gray-600 mb-4">{{product?.brandName}}</p>
                        <div class="mb-4">
                            <span class="text-2xl font-bold mr-2">{{ displayCurrency(product?.sellingPrice)}}</span>
                            <span class="text-gray-500 line-through">{{ displayCurrency(product?.price)}}</span>
                        </div>
        ...
        <categoryWiseProduct :category="product?.category.name" heading="Releted Product"/>
    </div>
</template>
<style scoped></style>